          ...
          <view class="comment-footer">
            ...
            <view class="right">
              ...
              <view class="like" @click.stop="changeLike(item)">
                <image class="icon" v-if="!item.praiseType" :src="'http://card-1254165941.cosgz.myqcloud.com/cardImages/descover/likeun.png'" ></image>
...
<script>
  ...
  export default {
    ...
    methods: {
      fetch () {
        this.loading = true;
        this.$api.getJournalComment(this.comment.id, this.currentPage).then(result => {
          this.loading = false;
      ...
      removeReply (reply, index) {
        const postDelete = () => {
          uni.showLoading();
          this.$api.deleteCommentReply(reply.id).then(result => {
            uni.hideLoading();
