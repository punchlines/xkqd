...
//         return data;
      ...
      commitByCOD () {
        ...
        const action = () => {
          ...
          this.$api.createOrder(data.addressId, data.items,data.msgs, 1).then((res)=>{
            uni.hideLoading()
            uni.redirectTo({url: '../paySuccess/paySuccess?type=cod'});
          }).catch((err)=>{
            uni.hideLoading()
            console.info(err)
            this.showError(err, '创建订单失败')
      ...
      getPayData(orderNum){
        this.$api.unifiedorder(orderNum).then(result => {
          ...
          uni.redirectTo({ url:'../paySuccess/paySuccess' });
        }).catch(error => {
					uni.hideLoading();
      ...
      //Vuex引入方法
      ...mapMutations(['setCardUserId','setUPinfo','setCarGoods','orderCouponTitle','orderCouponMoney', 'orderCouponId']),
    },
    computed: {
      //Vuex引入属性
