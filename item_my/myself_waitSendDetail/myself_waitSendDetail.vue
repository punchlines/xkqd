		...
		<view v-if="Detail">
			...
			<view class="ReceiveProduct">
				...
				<view class="OrderInfor fs6a24">
					...
					<view class="payTime" v-if="!isCOD">支付时间：{{payTime}}</view>
					<view class="payTime" v-else>支付时间：货到付款</view>
				</view>
			</view>
			<!-- 确认收货 -->
			<view class="confirmProduct">
				<view class="Button fx-row fx-row-right">
					<!-- <view class="AgreeReceive gray" @click="applyrefund(Detail.id)">申请退款</view> -->
					<!-- <view class="AgreeReceive" v-if="!isSO" @click="agreeAchieve(Detail.id)">确认收货</view> -->
					<view class="AgreeReceive" v-if="isSO" @click="goSendGood">去发货</view>
				</view>
			</view>
		</view>
	</view>
	...
	export default {
		...
		methods:{
			goSendGood(){
				...
				uni.navigateTo({
					url:'../myself_salesOrderSendsGoods/myself_salesOrderSendsGoods?data='+encodeURIComponent(data)
				})
			},
			...
			// 确认收货
			agreeAchieve(childId){
				this.$api.achieveOrder(childId).then(res=>{
					console.log(res);
					
					this.showTips('确认收货成功').then(res=>{
						this.updated();
						uni.navigateTo({
