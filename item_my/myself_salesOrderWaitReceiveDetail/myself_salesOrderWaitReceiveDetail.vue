		...
		<view v-for="(Detail,Dindex) in salesOrderSendD" :key="Dindex">
			...
			<view class="confirmProduct">
				<view class="Button fx-row fx-row-right">
					<view class="confirmGood" v-if="isCOD" @click="agreeReceive">确认用户已收货</view>
...
<script>
	...
	export default {
		...
		methods:{
			...
			// 待收货详情
			receiveSaleOrderDetail(){
				this.$api.receiveSaleOrderDetail(this.childId).then(res=>{
					console.log(res);
			...
			agreeReceive(){
				...
				this.$api.achieveOrder(this.childId).then(res=>{
					uni.hideLoading();
					uni.setStorageSync('_tempOrderId', this.childId);
					this.showTips('订单已完成').then(result => {
						uni.navigateBack();
					})
				}).catch(error => {
					this.showError(error)
	...
	.container{
		...
		.ReceiveProduct{
			...
			.PRrealPayment{
				...
				.picon{font-size: 26upx;}
				.price{font-size: 36upx;}
			}
			.bullPositionMessage{
