<template>
	<view class="container">
		...
		<view class="OrderHeaderBox">
			<view class="OHlist fx-row fx-row-center fx-row-space-around">
				<view :class="{'OHitem':true,'fs6a28':true,'OHactive':index==orderAction}" 
					v-for="(item,index) in orderHeader" :key="index" @tap="changeTitle(index)">
					{{item.title}}
				</view>
			</view>
		</view>
		...
		<view class="ALLproduct" v-if="orderAction==0">
			<AllOrder ref="list0" status="0"></AllOrder>
		</view>
		<!-- 待发货 -->
		<view class="WaitSend" v-if="orderAction==1">
			<AllOrder ref="list1"  status="1"></AllOrder>
			<!-- <WaitSend ref="list1"></WaitSend> -->
		</view>
		<!-- 待收货 -->
		<view class="WaitReceive" v-if="orderAction==2">
			<AllOrder ref="list2"  status="2"></AllOrder>
		</view>
		<!-- 待评价 -->
...
<script>
	...
	import WaitSend from '../myself_waitSend/myself_waitSend.vue';
	import WaitReceive from '../myself_waitReceive/myself_waitReceive.vue';
	import WaitEvaluate from '../myself_waitEvaluate/myself_waitEvaluate.vue';
	import AfterService from '../myself_AfterService/myself_AfterService.vue';
	export default {
		data() {
			return {
				// 订单导航
		...
		onShow() {
			if(uni.getStorageSync('_needUpdateShopOrder')){
