<template>
	<view class="container">
		<!-- 待发货订单列表 -->
		<view v-if="Alllist.length>0" class="AllOrderListBox" v-for="(item,index) in Alllist" :key="index">
			<view class="AOLlist">
				<view class="ListHeader fx-row fx-row-center fx-row-space-around">
					...
					<view class="LHorderState fs6a24" @click="drawAbill(item.orderId,item.payAmount,item.phone)">发票详情</view>
				</view>
			</view>
...
<script>
	export default {
		...
		methods:{
			drawAbill(orderId,payAmount,phone){
				uni.navigateTo({
					url: '../myself_invoiceDetails/myself_invoiceDetails?goodsId='+orderId+'&goodsAmount='+payAmount+'&phone='+phone
				});
			},
			//获取发票记录的商品列表
			getInvoice(){
				this.$api.getInvoiceMessage(2).then(res=>{
					console.log(res);
					if(res.invoiceDetail.length==0){
						this.showDefaultPage=true;
						return;
					}
					this.Alllist=res.invoiceDetail;
				}).catch(error=>{
