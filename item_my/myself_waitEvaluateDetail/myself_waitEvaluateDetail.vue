					...
					<view class="ProductList"  v-for="(item,index) in Detail.orderList" :key="index">
						<view class="ProductItem fx-row fx-row-center fx-row-space-around">
							...
							<view class="PIinformation">
								<view class="PFheader fx-row fx-row-top fx-row-left">
									<view class="Htitle fs3a28">{{item.goodsName?item.goodsName:""}}</view>
									<!-- v-if="showAfterSale" -->
									<view class="Hrefund fsf24" v-if="!isUseCoupon && !isCOD" @click="applyrefund(item)">申请售后</view>
								</view>
...
<script>
	...
	export default {
		...
		computed: {
			isUseCoupon () {
				return this.judgeOrderDetailList[0] && this.judgeOrderDetailList[0].preferentialMoney > 0
			}
		},
		...
		methods:{
			...
			fetch(){
				// this.childId
				this.showLoading();
				this.$api.judgeOrderDetail(this.childId).then(res=>{
					this.hideLoading();
					console.log(res);
...
<style lang="less">
	...
	.container{
		...
		.confirmProduct{
			...
			.Button{
				position: absolute;right:20upx;top:10%;text-align: center;line-height: 70upx;font-size:28upx;
				.checkLigistics{
					.buttonRadius(@w:180upx,@h:80upx,@bg:none);
