      ...
      <view class="storeListBox">
        <view class="SBlist fx-row fx-row-center fx-row-space-around" v-for="(item,index) in list" :key="index" @click="gotoMycard(item.id)">
          ...
          <view class="LTitle fs3a32 fx-row fx-row-left">
            <view class="LtitleName ">{{item.userName}}</view>
						 <view class="LtitleName" style="font-style: 13upx;white-space: nowrap">{{formatTimes[index]}}</view>
						 
          </view>
          <view class="applyBox">
            <vip-flag :type="item.userType"></vip-flag>
          </view>
        </view>
        <view v-if="noMore && this.list.length==0" class="default">
  ...
  export default {
    ...
    methods:{
      ...
      getCustomerList(){
        ...
        this.loading = true;
        this.showLoading();
        this.$api.getMyInviteVipList(this.currentPage).then(res=>{
          this.hideLoading();
          console.log(res)
          this.loading = false;
          ...
          this.list = this.list.concat(res);
					...
					res.map((item,index)=>{
							arr.push(this.formatTime(item.createTime));
						
					});
					this.formatTimes  = this.formatTimes.concat(arr);
					
					// this.list.map((item,index)=>{
					// 	 this.formatTimes.push(this.formatTime(item.createTime));
					// });
