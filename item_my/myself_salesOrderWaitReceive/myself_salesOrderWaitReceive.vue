	...
	<view class="container1">
		...
		<view class="AllOrderListBox" v-for="(item,index) in saleOrderWaitSend" :key="index">
			<!-- <view class="AllOrderListBox" v-for="(it,ind) in item.shopList" :key="ind"> -->
				...
				<view class="ListCenter" @click="gotoReceiveDetail(item.childId)">
				...
				<view class="sendProduct fx-row fx-row-right">
					<view class="SendButton fs6a24" @click="gotoLog(item.childId,item.expressNum,item.expressCompany)">查看物流</view>
				</view>
			<!-- </view> -->
		</view>
...
<script>
	...
	export default {
		...
		methods:{
			...
			saleGoodsStatus(){
				this.showLoading();
				this.$api.saleGoodsStatus(2,this.currentPage,20).then(res=>{
					this.hideLoading();
					this.loading = false;
