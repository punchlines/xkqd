...
<script>
  ...
  import price from '@/components/shop/_component/price';
  import NumberInput from "@/components/shop/_component/numberInput";
  ...
  export default {
    ...
    computed: {
			skuName(){
				let name = "";
				name = this.goodsSku.list.map(it=>{
						return it.sku.filter(its=>{
								return its.select?its:false;
						})[0].name;
      ...
      confirm () {
        ...
        this.$emit('confirm',{
          ...
          data: this.priceObj,
					...
					title:this.goodsSku.mpGoods.title
        })
        this.$emit('close');
      },
